"""
전략①: 테마 수급 모멘텀 (Theme Supply Momentum)
- 외국인 수급 유입 중이지만 가격은 아직 미반영된 테마
- 역발상 전략 (contrarian)

Usage:
  python py/strategy/theme/momentum.py analyze
  python py/strategy/theme/momentum.py backtest
"""
import sys
import os
...
def score_fn(groups, foreign_window, prices_pivot, window_dates):
    ...
    gs["supply_score"] = (
        gs["frgn_buy_ratio"].rank(pct=True) * 40
        + gs["frgn_buy_days_avg"].rank(pct=True) * 30
        + gs["frgn_net_avg"].rank(pct=True) * 30
    )
    gs["price_lag"] = gs["supply_score"].rank(pct=True) - gs["price_return_avg"].rank(pct=True)
    gs["score"] = gs["supply_score"] + gs["price_lag"] * 50
