...
// HNSExample 은 해드앤숄더 매도 신호 1건 (+ forward return + 패턴 속성).
type HNSExample struct {
	Market      string `json:"market"`
	Shcode      string `json:"shcode"`
	TriggerDate string `json:"trigger_date"` // 넥라인 이탈 캔들
	ArmDate     string `json:"arm_date"`     // 오른어깨 인지 캔들
	LSDate      string `json:"ls_date"`
	HDate       string `json:"h_date"`
	RSDate      string `json:"rs_date"`
	ArmToTrig   int    `json:"arm_to_trigger_bars"`
	// 사후 분석용 패턴 속성
	ShoulderDiffPct float64 `json:"shoulder_diff_pct"`
	HeadPromPct     float64 `json:"head_prom_pct"`
	NeckSlopePct    float64 `json:"neck_slope_pct"`
	VolumeRatio     float64 `json:"volume_ratio"` // <1 이면 정식 거래량 감소 충족
...
// HandleHNSScan 은 전 종목에서 해드앤숄더 매도 신호를 수집하고 음의 엣지를 측정한다.
// 사용법: ./RESTGo stock hns_scan [--foreign-jp|--foreign-cn|--foreign-hk] [--max N] [--candles N] [--out path]
// 기본: hannam 16년(4200봉), 출력 zpicture/hns_scan.json
func HandleHNSScan(args []string) {
	mode := "hannam"
	maxStocks := 0
	candleCount := 4200
	outPath := "zpicture/hns_scan.json"
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[hns_scan] DB 연결 오류: %v\n", err)
		return
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[hns_scan] 종목 목록 오류: %v\n", err)
		return
	}
	...
	fmt.Printf("[hns_scan] %s 모드 %d개 종목, 캔들 %d\n", market, len(stocks), candleCount)
	...
	for _, code := range stocks {
		...
		go func(shcode string) {
			...
			signals := stg.HNSAnalyze(candles)
			...
			var localEx []HNSExample
			for _, s := range signals {
				localEx = append(localEx, HNSExample{
					Market: market, Shcode: shcode,
					TriggerDate: s.Date, ArmDate: s.ArmDate,
					LSDate: candles[s.Pattern.LSPos].Date,
					HDate:  candles[s.Pattern.HPos].Date,
					RSDate: candles[s.Pattern.RSPos].Date,
					ArmToTrig:       s.Pos - s.ArmPos,
					ShoulderDiffPct: s.Pattern.ShoulderDiffPct,
					HeadPromPct:     s.Pattern.HeadPromPct,
					NeckSlopePct:    s.Pattern.NeckSlopePct,
					VolumeRatio:     s.Pattern.VolumeRatio,
					PatternWidth:    s.Pattern.PatternWidth,
			...
			if n := scanned.Add(1); n%500 == 0 {
				fmt.Printf("[hns_scan] 진행 %d/%d (신호 %d)\n", n, len(stocks), len(examples))
			}
		}(code)
	}
	wg.Wait()
	...
	for _, h := range mtopHorizons {
		...
		stats = append(stats, hStat{
			...
			DownRate: 100 * float64(down) / float64(len(rs)), BaselineMean: bm,
	...
	out := struct {
		...
		Examples    []HNSExample `json:"examples"`
	}{market, candleCount, len(stocks), len(examples), stats, examples}

	data, _ := json.MarshalIndent(out, "", "  ")
	if err := os.WriteFile(outPath, data, 0644); err != nil {
		fmt.Fprintf(os.Stderr, "[hns_scan] 저장 실패: %v\n", err)
		return
	}

	fmt.Printf("[hns_scan] 완료: 종목 %d(실패 %d)  신호 %d  → %s\n",
		scanned.Load(), failed.Load(), len(examples), outPath)
	for _, s := range stats {
		fmt.Printf("[hns_scan] h%-2d n=%d  mean %+.3f%%  median %+.3f%%  하락률 %.1f%%  baseline %+.3f%%  edge %+.3f%%p  t=%.2f\n",
